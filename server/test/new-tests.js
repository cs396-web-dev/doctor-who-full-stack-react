...
const areSameCompanionsInBothArrays = function (actual, expected) {
    const matched = actual.filter(a => {
        const matches = expected.filter(b => {
            return a.name == b.name && 
                ...
                JSON.stringify(a.image_url) === JSON.stringify(b.image_url)
        });
...
describe("[NEW] Updated /companions/:id", function () {
    ...
    describe("PATCH", () => {
        ...
        it("patch honors image_url", done => {
            const _id = "" + fixtures.companion._id;
